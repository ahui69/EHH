#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Monolit PRO — Modularized Architecture
# Version 3.3.0 - Refactored with core modules

import os, re, sys, time, json, uuid, sqlite3, asyncio, contextlib, subprocess, threading
from dotenv import load_dotenv
load_dotenv()  # Ładuj .env
import datetime, html, unicodedata, dataclasses, hashlib, math
...
from core.helpers import (
    ...
    tfidf_vec as _tfidf_vec, tfidf_cosine as _tfidf_cos,
    cosine_similarity as _vec_cos, sentences_split as _sentences,
    tag_pii as _tag_pii_in_text, extract_profile_info,
    parse_query_string as _qs, read_json_body as _read_json,
    get_cors_headers as _cors, json_response as _json, error_response as _bad
)
from core.llm import call_llm, call_llm_once, call_llm_stream, sanitize_llm_response
from core.memory import (
    memory_manager, time_manager,
    STM_CONTEXT, LTM_FACTS_CACHE, LTM_CACHE_LOADED,
    AdvancedMemoryManager, TimeManager,
    _db, _init_db, _preload_seed_facts,
    ltm_add, ltm_delete, ltm_search_hybrid, ltm_search_bm25, ltm_reindex,
    stm_add, stm_get_context, stm_clear,
    memory_add, memory_get, memory_summaries, memory_purge,
    _extract_facts, _blend_scores, facts_reindex, system_stats,
    psy_get, psy_set, psy_episode_add, psy_observe_text, psy_reflect, psy_tune, psy_tick,
    load_ltm_to_memory, _ltm_search_from_cache
)
from core.tools import (
    InternetSearcher, internet_searcher,
    tools_time_handler, tools_search_handler, tools_news_handler,
    tools_sports_handler, tools_news_digest_handler
)
...
app = FastAPI(title="Mordzix Monolit PRO", version="3.3.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], allow_credentials=True,
    allow_methods=["*"], allow_headers=["*"],
)
# --- Research API (autonauka/web) ---
from fastapi import Query, Depends, HTTPException

@app.get("/api/research/sources")
async def api_research_sources(q: str = Query(..., min_length=2), topk: int = 8, deep: bool = False, _=Depends(_auth_dep)):
    """Zwraca kontekst i źródła dla zapytania (autonauka)."""
    try:
        # autonauka może być sync lub async – obsłuż obie ścieżki
        if asyncio.iscoroutinefunction(autonauka):
            res = await autonauka(q, topk=topk, deep_research=deep)
        else:
            loop = asyncio.get_event_loop()
            res = await loop.run_in_executor(None, lambda: autonauka(q, topk=topk, deep_research=deep))
        return {"ok": True, **(res or {})}
    except Exception as e:
        log_error(e, "API_RESEARCH")
...
# --- MEMORY SYSTEM ---
# Ulepszone zarządzanie pamięcią STM/LTM z zaawansowanym kontekstem
# (CONFIG, SYSTEM_PROMPT, CONTEXT_DICTIONARIES importowane z core.config)
app = FastAPI()

# Short Term Memory
STM_CONTEXT: list[dict] = []  # ostatnie wiadomości w RAM

# Long Term Memory
LTM_FACTS_CACHE: list[dict] = []  # fakty w RAM
LTM_CACHE_LOADED = False
LTM_FACTS_CACHE: list[dict] = []  # fakty w RAM
LTM_CACHE_LOADED = False


class AdvancedMemoryManager: """Zaawansowany 
    menedżer pamięci z analizą kontekstu"""
class ChatMessage(BaseModel): 
    ...
    self.stm_limit = 130 # Zwiększony limit 
    STM user_id: str 
    self.ltm_importance_threshold = 0.7 # 
    Próg ważności dla LTM use_memory: bool = 
    True self.context_window = 45 # Okno 
    kontekstu dla analizy auto_learn: bool = 
    True def add_to_stm(self, message: dict, 
    user_id: str = "default"):
        """Dodaj wiadomość do STM z analizą 
        kontekstu""" if user_id not in 
        STM_CONTEXT:
@app.post("/api/chat/assistant") 
STM_CONTEXT[user_id] = [] async def 
chat_assistant(data: ChatMessage, request: 
Request):
    return {"answer": f"Odebrałem: {len(data.messages)} wiadomości od {data.user_id}"}        # Analiza kontekstu wiadomości
    ...
    def _analyze_context(self, message: dict) -> dict:
        ...
        context_score = {"technical": 0, "casual": 0, "sports": 0, "other": 0}
                ...
                for word in words:
                    if word.lower() in content:
                        if category == "technical":
                            context_score["technical"] += 1
                        elif category == "casual":
                            context_score["casual"] += 1
                        elif category == "sports":
                            context_score["sports"] += 1
                        else:
                            context_score["other"] += 1
    ...
    def _promote_to_ltm(self, message: dict, user_id: str):
        """Przenieś ważną wiadomość do LTM (bez placeholderów)."""
        try:
...
# --- UTILS/TOOLS ENDPOINTS (REAL CAPABILITIES) ---
@app.get("/api/tools/time")
async def tools_time():
...
async def tools_search(q: str, limit: int = 5):
    results = await internet_searcher.search_web(q, limit=limit)
    return {"ok": True, "query": q, "results": results}

@app.get("/api/tools/news")
async def tools_news(limit: int = 6):
    try:
...
async def tools_sports(kind: str = "epl", limit: int = 8):
    try:
        import sports_news_pro as SNP
...
@app.get("/api/tools/news_digest")
async def tools_news_digest(topic: str = "przegląd dnia", limit: int = 6):
    try:
...
@app.post("/api/learn")
async def api_learn(body: LearnReq):
    """Zainicjuj proces nauki: web_learn/autonauka i zwróć efekty."""
    try:
        q = (body.topic or "").strip()
        if not q:
            return {"ok": False, "error": "topic required"}
        # prefer web_learn jeśli dostępne
        if 'web_learn' in globals():
            res = web_learn(q, mode=body.mode)
            return {"ok": True, "data": res}
        # fallback do autonauka (async)
        if 'autonauka' in globals():
            data = await autonauka(q, topk=12, deep_research=body.deep)
            return {"ok": True, "data": data}
        return {"ok": False, "error": "no learn backend"}
    except Exception as e:
        log_error(e, "API_LEARN")
        return {"ok": False, "error": str(e)}

# --- LTM PUBLIC ENDPOINTS ---
class LtmAddReq(BaseModel):
    text: str
    tags: str = ""
    conf: float = 0.7

@app.post("/api/ltm/add")
async def api_ltm_add(body: LtmAddReq):
    try:
        if not (body.text or "").strip():
            return {"ok": False, "error": "text required"}
        tid = ltm_add(body.text.strip(), body.tags or "", float(body.conf or 0.7))
        return {"ok": True, "id": tid}
    except Exception as e:
        log_error(e, "API_LTM_ADD")
        return {"ok": False, "error": str(e)}

@app.get("/api/ltm/search")
async def api_ltm_search(q: str, limit: int = 30):
    try:
        items = ltm_search_hybrid(q, limit=limit) or []
        return {"ok": True, "items": items[:limit]}
    except Exception as e:
        log_error(e, "API_LTM_SEARCH")
        return {"ok": False, "error": str(e), "items": []}

# --- MEMORY EXPORT/IMPORT (NO PLACEHOLDERS) ---
@app.get("/api/memory/export")
async def memory_export(limit: int = 1000):
    conn = _db(); conn.row_factory = sqlite3.Row
    try:
        c = conn.cursor()
        rows = c.execute(
            """
            SELECT id, text, tags, conf, created AS created_at
            FROM facts
            WHERE deleted = 0
...
async def memory_import(payload: dict):
    items = payload.get("items") or []
    if not isinstance(items, list) or not items:
        return {"ok": False, "error": "Brak elementów do importu"}
    ...
    for it in items:
        ...
        if not text:
            continue
        tags = (it or {}).get("tags") or ""
        conf = float((it or {}).get("conf") or 0.7)
        tid = _id_for(text)
        ts = time.time()
        try:
            c.execute(
                "INSERT OR REPLACE INTO facts(id,text,tags,conf,created,deleted) VALUES(?,?,?,?,?,0)",
                (tid, text, tags, conf, ts)
            )
            try:
                c.execute("INSERT INTO facts_fts(text,tags) VALUES(?,?)", (text, tags))
            except Exception:
                # brak indeksu FTS nie blokuje importu
                pass
            inserted += 1
        except Exception:
...
def load_ltm_to_memory():
    """Wczytaj wszystkie fakty z SQLite do RAM przy starcie z zaawansowaną analizą"""
    global LTM_FACTS_CACHE, LTM_CACHE_LOADED

    if LTM_CACHE_LOADED:
        log_info("LTM cache already loaded, skipping", "LTM_CACHE")
        return

    try:
        db_path = DB_PATH
        log_info(f"Loading advanced LTM cache from {db_path}", "LTM_CACHE")

        if not os.path.exists(db_path):
            log_warning(f"Database {db_path} does not exist - LTM cache empty", "LTM_CACHE")
        ...
        # Sprawdź czy tabela istnieje (facts)
        tables = cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='facts'").fetchall()
        if not tables:
            log_warning("facts table does not exist - LTM cache empty", "LTM_CACHE")
            conn.close()
            LTM_CACHE_LOADED = True
            return

        # Wczytaj WSZYSTKIE fakty z zaawansowaną analizą
        try:
            # Dodajemy analizę kontekstu do zapytań
            rows = cur.execute("""
                SELECT id, text, tags, '' as source, conf, created as created_at,
            ...
            log_error(db_error, "LTM_DB_QUERY")
            # Fallback do prostszego zapytania
            rows = cur.execute("SELECT id, text, tags, '' as source, conf, created as created_at FROM facts WHERE deleted=0 LIMIT 20000").fetchall()

        facts_loaded = 0
        context_stats = {"technical": 0, "casual": 0, "sports": 0, "other": 0}

        for row in rows:
            try:
                # Analiza kontekstu dla każdego faktu
                fact_text = row[1] or ""
                temp_message = {"content": fact_text}
                context_analysis = memory_manager._analyze_context(temp_message)

                # Ulepszony fakt z metadanymi
                enhanced_fact = {
                    'id': row[0],
                    'text': fact_text,
                    'tags': row[2] or '',
                    ...
                    'tokens': _tok(fact_text) if fact_text else 0,
                    'context_analysis': context_analysis,
                    'importance_score': memory_manager._calculate_importance(temp_message, context_analysis),
                    'text_length': len(fact_text),
                    'tag_frequency': row[6] if len(row) > 6 else 1
                }

                LTM_FACTS_CACHE.append(enhanced_fact)
                facts_loaded += 1

                # Statystyki kontekstu
                dominant = context_analysis.get("dominant", "other")
                if dominant in context_stats:
                    context_stats[dominant] += 1

            except Exception as fact_error:
                log_warning(f"Error processing fact {row[0]}: {fact_error}", "LTM_FACT_PROCESSING")
                continue
        ...
        # Loguj statystyki
        log_info(f"Advanced LTM cache loaded: {facts_loaded} facts", "LTM_CACHE")
        log_info(f"Context distribution: {context_stats}", "LTM_CACHE")
        log_info(f"Total facts in memory: {len(LTM_FACTS_CACHE)}", "LTM_CACHE")

        # Sortuj fakty według ważności dla szybszego dostępu
        LTM_FACTS_CACHE.sort(key=lambda x: x.get('importance_score', 0), reverse=True)

    except Exception as e:
        log_error(e, "LTM_CACHE_LOAD")
        LTM_CACHE_LOADED = True  # Set to True to prevent infinite retry loops

# Serve frontend FIRST (before routers)
from fastapi.responses import FileResponse, HTMLResponse
from fastapi import Depends

# Prosty logger-safe (zanim powstaną właściwe utilsy)
# log_info, log_warning, log_error imported from core.helpers

@app.get("/", response_class=HTMLResponse)
async def serve_frontend():
    """Serve the main frontend HTML from correct path."""
    dist_index = os.path.join(os.getenv("WORKSPACE", "/workspace/mrd"), "dist", "index.html")
    candidates = [
        os.getenv("FRONTEND_INDEX") or "",
        dist_index,
        "/workspace/mrd/dist/index.html",
        "/workspace/mrd/frontend.html",
        "/workspace/mrd/frontend/index.html",
        "/workspace/frontend.html",
    ]
    for path in candidates:
        ...
        try:
            ...
            continue
    return HTMLResponse(content="<h1>Frontend not found. Put frontend.html in /workspace/mrd</h1>", status_code=404)

@app.get("/api/health")
async def api_health():
    return {"ok": True, "uptime_s": int(time.time() - _START_TIME)}

# PWA assets (serve from project root)
@app.get("/sw.js", include_in_schema=False)
async def pwa_sw_js():
    # Prefer dist/sw.js, then root sw.js
    dist_path = os.path.join(BASE_DIR, "dist", "sw.js")
    path = dist_path if os.path.isfile(dist_path) else os.path.join(BASE_DIR, "sw.js")
...
async def pwa_manifest():
    # Prefer dist/manifest.webmanifest, then root
    dist_path = os.path.join(BASE_DIR, "dist", "manifest.webmanifest")
    path = dist_path if os.path.isfile(dist_path) else os.path.join(BASE_DIR, "manifest.webmanifest")
    if os.path.isfile(path):
...
async def pwa_icon_192():
    path = os.path.join(BASE_DIR, "pwa-icon-192.png")
    if os.path.isfile(path):
...
async def pwa_icon_512():
    path = os.path.join(BASE_DIR, "pwa-icon-512.png")
    if os.path.isfile(path):
        return FileResponse(path, media_type="image/png")
    return HTMLResponse(status_code=404, content="icon not found")

# Serve built assets from Vite (dist/assets)
@app.get("/assets/{path:path}", include_in_schema=False)
async def serve_assets(path: str):
    base = os.path.join(BASE_DIR, "dist", "assets")
    target = os.path.join(base, path)
    if os.path.isfile(target):
        # Guess minimal media type by extension
        mt = "application/octet-stream"
        try:
            import mimetypes
            mt = mimetypes.guess_type(target)[0] or mt
        except Exception:
            pass
        return FileResponse(target, media_type=mt)
    return HTMLResponse(status_code=404, content="asset not found")

# Favicon (prefer dist/favicon.ico)
@app.get("/favicon.ico", include_in_schema=False)
async def serve_favicon():
    dist_fav = os.path.join(BASE_DIR, "dist", "favicon.ico")
    root_fav = os.path.join(BASE_DIR, "favicon.ico")
    if os.path.isfile(dist_fav):
        return FileResponse(dist_fav, media_type="image/x-icon")
    if os.path.isfile(root_fav):
        return FileResponse(root_fav, media_type="image/x-icon")
    # fallback to pwa icon
    pwa = os.path.join(BASE_DIR, "pwa-icon-192.png")
    if os.path.isfile(pwa):
        return FileResponse(pwa, media_type="image/png")
...
# Psyche Endpoint (stan psychiczny AI) - USES core/memory.py
try:
    import psyche_endpoint
    app.include_router(psyche_endpoint.router)
    print("[OK] Psyche endpoint loaded - /api/psyche/*")
except Exception as e:
    print(f"[ERROR] psyche_endpoint failed: {e}")

# Programista Endpoint (code execution) - USES core/executor.py
try:
    import programista_endpoint
    app.include_router(programista_endpoint.router)
    print("[OK] Programista endpoint loaded - /api/code/*")
except Exception as e:
    print(f"[ERROR] programista_endpoint failed: {e}")
...
async def startup_event():
    ...
    load_ltm_to_memory()
    # Import facts_5000.json jeżeli baza jest pusta lub bardzo mała
    try:
        conn=_db(); c=conn.cursor(); cnt=c.execute("SELECT COUNT(*) FROM facts").fetchone()[0]; conn.close()
        if cnt < 100:
            path = os.path.join(BASE_DIR, "facts_5000.json")
            if os.path.isfile(path):
                print(f"[BOOT] Importuję fakty z {path}…")
                res = import_facts_json(path, max_items=200000)
                print(f"[BOOT] Import wynik: {res}")
                # po imporcie doładuj cache LTM
                global LTM_CACHE_LOADED, LTM_FACTS_CACHE
                LTM_FACTS_CACHE=[]; LTM_CACHE_LOADED=False
                load_ltm_to_memory()
    except Exception as e:
        print(f"[BOOT] Błąd importu facts_5000.json: {e}")
...
try:
    ...
    # Twardy fallback bez atrapy: podnieś błąd, żeby nie maskować problemów
    def _not_available(*a, **k):
        raise RuntimeError("LTM backend not available - sprawdź inicjalizację bazy i modułów LTM")
    ltm_add = _not_available
    ltm_search_hybrid = _not_available
    ltm_search_tags = _not_available
    memory_get = _not_available
...
# =========================
# KONFIG (AUTH_TOKEN moved to top, see line ~94)
# =========================
# BASE_DIR, DB_PATH, etc. already defined above
...
LLM_TIMEOUT  = int(os.getenv("LLM_HTTP_TIMEOUT_S", "45"))
...
SEED_CANDIDATES = [
    ...
    "/workspace/mrd/data/sq3 seed_facts.jsonl",
    "/workspace/mrd/data/sq3_seed_facts.jsonl",
    "/workspace/mrd/seed_facts.jsonl",
...
# Flaga dostępności FTS5 (ustawiana w _init_db)
FTS5_AVAILABLE = True
...
def _db():
    ...
    try:
        # Większy cache w RAM i mmap dla szybszych odczytów (bez utraty danych)
...
def _init_db():
    ...
    try:
        ...
        # Brak wsparcia FTS5 w sqlite – przechodzimy w tryb degradacji
        FTS5_AVAILABLE = False
        print(f"[WARN] FTS5 not available ({e}). Using fallback LIKE search.")
...
def _preload_seed_facts():
    ...
    # Dodatkowo: załaduj /workspace/mrd/facts_5000.json jeśli istnieje (format JSON Array)
    try:
        json_path = os.path.join(BASE_DIR, "facts_5000.json")
        if os.path.isfile(json_path):
            print(f"Ładuję fakty z JSON: {json_path}")
            with open(json_path, "r", encoding="utf-8") as jf:
                data = json.load(jf)
                added = 0
                for obj in data[:50000]:
                    try:
                        txt = obj.get("text") or obj.get("fact")
                        if not txt: continue
                        tags = obj.get("tags") or []
                        if isinstance(tags, list): tags = ",".join(tags)
                        conf = float(obj.get("conf") or obj.get("confidence") or 0.85)
                        ltm_add(txt, tags, conf)
                        added += 1
                    except Exception:
                        continue
                print(f"Dodano z JSON: {added} faktów")
    except Exception as e:
        print(f"Błąd ładowania facts_5000.json: {e}")
...
def ltm_search_hybrid(q: str, limit: int = 30)->List[Dict[str,Any]]:
    """Hybrid search: RAM cache → FTS5 BM25 → LIKE/TFIDF na tabeli 'facts'."""
...
LLM_FALLBACK_MODEL = os.getenv("LLM_FALLBACK_MODEL", "zai-org/GLM-4.5")
LLM_TIMEOUT = int(os.getenv("LLM_TIMEOUT", "45"))

def _llm_request(messages: list[dict], model: str, **opts) -> str:
    """Wysyła żądanie do DeepInfra z retry/backoff i krótszym timeoutem.
    Obsługuje opcje: temperature, max_tokens, timeout_s.
    """
            ...
            if result.get("draft"):
                ...
                for part in draft_parts:
                    if part.startswith("- ") and len(part) > 30:
                        fact_medium_highlights.append(part[2:]) # Usuń "- " z początku
            
            out = {
                "query": q,
                "context": "\n\n".join(ctx[:topk]),
                "facts": fact_medium_highlights[:5],
...
def import_facts_json(json_path: str, max_items: int = 200000) -> Dict[str, Any]:
    """Załaduj fakty z pliku JSON (lista obiektów) do tabeli facts.
    Każdy obiekt powinien mieć co najmniej pole "text".
    """
    total = 0
    if not os.path.isfile(json_path):
        return {"ok": False, "error": "file_not_found", "path": json_path}
    try:
        with open(json_path, "r", encoding="utf-8") as jf:
            data = json.load(jf)
        for obj in data[:max_items]:
            try:
                txt = (obj.get("text") or obj.get("fact") or "").strip()
                if not txt:
                    continue
                tags = obj.get("tags") or []
                if isinstance(tags, list):
                ...
                continue
        # Przeindeksuj jeśli dostępny FTS5
        try:
            _ = facts_reindex()
        except Exception:
            pass
        return {"ok": True, "loaded": total, "path": json_path}
    except Exception as e:
        return {"ok": False, "error": str(e), "path": json_path}
...
# =========================
def _compress_response(text: str, max_len: int = 50000) -> str:
...
def wsgi_app_disabled(env, start_response):  # disabled by patch
    ...
    if m=="GET" and p=="/api/health":
        ...
        if m=="POST" and p=="/api/chat/complete":
            ...
            if len(ans) > 50000:
                ans = _compress_response(ans, 50000)
...
app = FastAPI(title="Mordzix API", version="5.0")
...
async def chat_assistant(req: ChatRequest):
    """Prosty testowy endpoint, aby frontend przestał zwracać 405"""
    last = req.messages[-1].content if req.messages else "(pusty)"
    return {"answer": f"Odebrałem: {last}"}
